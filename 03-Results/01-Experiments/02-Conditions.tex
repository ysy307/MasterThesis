...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=.95\linewidth,pagebox=cropbox,clip]{3-1/initial_experiment_comparison.pdf}
...
その結果，完全流体におけるポテンシャル流れを用いているため，凍結管周りの粘性境界条件の影響で残差が少し大きいが，残差の最大ノルム および二乗平均平方根誤差はそれぞれ\num{2.012e-04}，\num{2.917e-05} となり，両者は良好に一致していることが確認され，初期圧力分布の設定が妥当であることが示された．

\subsubsection{使用した物性値}
\label{Sec:exp_material_properties}

本検証解析においては，実験に用いた鳥取砂丘砂の物理的性質を忠実に再現するため，予備実験によって決定した物性値を\cref{Table:exp_properties}に示した．また，これらの物性値に基づく水分保持関数を\cref{Fig:tottori_dune_sand_wrf}に示した．
\begin{table}[btp]
  \centering
  \caption{予備実験により同定された熱物性値および水理定数}
  \label{Table:exp_properties}
...
\begin{figure}[b]
  \centering
  \includegraphics[width=.85\linewidth, pagebox=cropbox, clip]{3-1/wrf_durner_tds.pdf}
  \caption{同定されたパラメータに基づく鳥取砂丘砂の水分保持関数}
  \label{Fig:tottori_dune_sand_wrf}
\end{figure}
