\subsection{直線状配置された凍結管における限界流速の同定}
\label{Sec:Param_Results_LinearPipes}
...
\label{Sec:Param_Results_Fitting}
...
\label{Sec:Param_Results_RepresentativeLength}

代表長さ $l$ の一般化された予測モデルを導出するため，\cref{Table:regression_models} に示すように，複数の回帰モデルを検討した．各モデルの説明変数，適用範囲，決定係数 ($R^2$)，および主要な知見を以下にまとめる．
...
\subsection{円形配置された凍結管における限界流速の同定}
\label{Sec:Param_Results_CircularPipes}

本節では，\textcite{Takashi-1969}による限界流速の解析モデルに基づき，円形配置における凍土壁の閉塞限界について検討する．
\textcite{Takashi-1969}内では直線配置した際と同様に，円状に配置した際の限界流速式も次式で提案されている．
\begin{equation}
...
\end{equation}
ここで，$r$ は凍結管の配置に関する代表長さであり，本研究では配置半径とした．直線配置した際の限界流速式（\eqref{Eq:Takashi_critical_velocity_Linear}）と比較すれば，ポテンシャル流れが異なるため，分母の長差尺度代表長さが代表半径（配置半径） $r$ に変化し，定数がかかっている点が異なる．
...
配置半径が $r=\qty{2.0}{\meter}$ の場合，理論上の限界流速は約 \qty{0.73}{\meter.\day^{-1}} である．
しかし数値解析の結果，これを上回る初期流速 \qty{0.8}{\meter.\day^{-1}} においても凍土壁の閉塞が確認された．
通常，地下水流による熱移流は凍結を阻害する要因となる．
しかしながら，配置半径が小さい本ケースにおいては，上流部で冷却された間隙水が移流によって下流側の凍結管群へ到達し，初期温度よりも低い水が供給されることで，かえって凍土壁の成長が促進されたと考えられる．
その結果，\cref{Fig:2m_02md,Fig:2m_06md} に示すような閉塞順序の逆転現象が観測された．
流速 \qty{0.2}{\meter.\day^{-1}} では下流側が最後に閉塞したのに対し，流速 \qty{0.8}{\meter.\day^{-1}} では上流側が最後に閉塞している．
このことから，小半径かつ高流速の条件下では，移流成分が下流側の温度低下を促進するように作用し，理論値を超える流速での閉塞を可能にしたと結論付けられる．

この傾向は，配置半径が $r=\qty{4.5}{\meter}$ の場合（理論限界流速：約 \qty{0.32}{\meter.\day^{-1}}）においても同様である．
\cref{Fig:4.5m_03md,Fig:4.5m_04md} に示すように，この限界流速近傍を境界として凍土壁の閉塞形態が変化し，下流側閉塞型から上流側閉塞型への明確な移行が確認された．

一方，配置半径が $r=\qty{10.0}{\meter}$ の場合，限界流速は約 \qty{0.15}{\meter.\day^{-1}} まで低下する．
半径が大きくなると，上流部での凍結による流速遮断効果が下流まで及びにくく，また移流による低温水の輸送効果も希薄化する．
そのため，限界流速に近い \qty{0.2}{\meter.\day^{-1}} を超える初期流速では，凍土壁は閉塞しないと判定された．
閉塞に至った事例（初期流速 \qtylist{0.0;0.1;0.2}{\meter.\day^{-1}}）を \cref{Fig:10.0m_01md,Fig:10.0m_02md} に示す．
これらを見ると，小半径のケースと比較して，各凍結管を中心とした同心円状に凍結前線が成長しており，熱拡散が支配的な挙動を示していることがわかる．

凍結管を直線状配置した際（\cref{Table:Comparison_Critical_Velocities}）と異なり，凍結管を円形配置した時は，数値解析によって限界流速が理論値と比較して大きく乖離する傾向が見られた．
そのため，\cref{Sec:Param_Conditions_Linear}で示したような，数値解析結果に基づく限界流速の同定は困難であった．
