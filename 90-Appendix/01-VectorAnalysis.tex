...
\begin{proof}
  ...
  \end{align}
  Kronecker-deltaの定義に注意すれば，\eqref{eq:Levi-Civita-Identity-proof-5}の右辺は次のように書き換えることができる．
  \begin{subequations}
    \begin{equation}
...
\begin{FormulaBox}{ガウスの発散定理（3次元）}{Gauss-Divergence-Theorem-3D}
  ...
  \end{equation}
  ここで，$\vect{n}$は閉曲面$\partial V$上の外向き単位法線ベクトル，$dV$は体積要素，$\odif{S}$は面積要素である．
\end{FormulaBox}

\begin{proof}
  領域$V$が$x, y, z$の各軸方向に関して単純な領域（任意の軸に平行な直線が，境界と高々2点で交わる領域）であると仮定する．
  まず，$\nabla\cdot\vect{a}$の第3項（$z$微分成分）の体積積分を考える．$V$の$xy$平面への正射影を$D$とし，$V$の境界$\partial V$のうち，上側の面を$z=z_\mathrm{top}\pab{x,y}$，下側の面を$z=z_{btm}\pab{x,y}$とする．
  \begin{align}
    \label{eq:Gauss-Proof-3D-1}
    \int_{V} \frac{\partial a_3}{\partial z}  dV 
     & = \iint_{D} \bab{\int_{z_{btm}\pab{x,y}}^{z_\mathrm{top}\pab{x,y}} \frac{\partial a_3}{\partial z} \odif{z}}  \odif{x}\odif{y} \notag \\
     & = \iint_{D} \bab{a_3\pab{x,y,z_\mathrm{top}} - a_3\pab{x,y,z_{btm}}}  \odif{x}\odif{y}
  \end{align}
  ここで，上側の面における外向き法線ベクトル$\vect{n}_\mathrm{top}$と$\vect{e}_z$のなす角を$\gamma_\mathrm{top}$とすると，面素の関係$\odif{x}\odif{y} = \cos\gamma_\mathrm{top} \odif{S} = \pab{\vect{n}_\mathrm{top}\cdot\vect{e}_z}\odif{S}$が成り立つ．同様に下側の面では，外向き法線$\vect{n}_{btm}$が下を向くため，$\vect{n}_{btm}\cdot\vect{e}_z$は負となり，$\odif{x}\odif{y} = -\cos\gamma_{btm} \odif{S} = -\pab{\vect{n}_{btm}\cdot\vect{e}_z}\odif{S}$となる．
  これらを\eqref{eq:Gauss-Proof-3D-1}に代入し，閉曲面$\partial V$全体での積分に書き換えると，
  \begin{align}
    \label{eq:Gauss-Proof-3D-2}
    \iint_{D} a_3\pab{x,y,z_\mathrm{top}}  \odif{x}\odif{y} + \iint_{D} -a_3\pab{x,y,z_{btm}} \odif{x}\odif{y} \notag                                                    \\
     & = \int_{\partial V_\mathrm{top}} a_3 \pab{\vect{n}\cdot\vect{e}_z}  \odif{S} + \int_{\partial V_\mathrm  {btm}} a_3 \pab{\vect{n}\cdot\vect{e}_z} \odif{S} \notag \\
     & = \int_{\partial V} a_3 n_3  \odif{S}
  \end{align}
  となる．$x, y$成分についても同様の議論を行うことで，
  \begin{equation}
    \int_{V} \frac{\partial a_1}{\partial x}  dV = \int_{\partial V} a_1 n_1  \odif{S}, \quad
    \int_{V} \frac{\partial a_2}{\partial y}  dV = \int_{\partial V} a_2 n_2  \odif{S}
  \end{equation}
...
\begin{FormulaBox}{ガウスの発散定理（2次元）}{Gauss-Divergence-Theorem-2D}
  ...
  \begin{equation}
    ...
    \int_{S} \nabla\cdot\vect{a} \odif{S} = \oint_{\partial S} \vect{a}\cdot\vect{n}  \odif{l}
  \end{equation}
  ここで，$\nabla\cdot\vect{a} = \pdv{a_1}/{x} + \pdv{a_2}/{y}$であり，$\vect{n}$は境界$\partial S$上の外向き単位法線ベクトル，$l$は弧長パラメータである．
\end{FormulaBox}

\begin{proof}
  3次元の場合と同様に，$S$を$y$軸に平行な線分で切ったときに境界と高々2点で交わる単純な領域と仮定する．$\nabla\cdot\vect{a}$の第2項（$y$微分成分）の面積分を考える．
  境界$\partial S$を，上側の曲線$y=y_\mathrm{top}\pab{x}$と下側の曲線$y=y_\mathrm{btm}\pab{x}$（定義域は$x_1 \le x \le x_2$）に分割する．
  \begin{align}
    \label{eq:Gauss-Proof-2D-1}
    \int_{S} \frac{\partial a_2}{\partial y} \odif{S}
     & = \int_{x_1}^{x_2} \bab{\int_{y_\mathrm{btm}\pab{x}}^{y_\mathrm{top}\pab{x}} \pdv{a_2}{y} \odif{y}}  \odif{x} \notag \\
     & = \int_{x_1}^{x_2} \bab{a_2\pab{x,y_\mathrm{top}} - a_2\pab{x,y_\mathrm{btm}}} \odif{x}
  \end{align}
  ここで，境界上の線素ベクトル$d\vect{l}$を反時計回りに取ると，外向き法線ベクトル$\vect{n}$を用いて$\vect{n}\odif{l} = \pab{\odif{y}, -\odif{x}}^\top$の関係がある（．
  上側の曲線では$x$が増加するにつれて積分路は右へ進むが，$\vect{n}$の$y$成分は正であるため，$\displaystyle \odif{x} = \odv{x}{l}\odif{l}$に対し$n_2 \odif{l} \approx \odif{x}$の関係が成り立つ．下側の曲線では積分路の向きと$x$の増分が逆になること等を考慮し，線積分へ書き換えると以下のようになる．
  \begin{align}=
    \int_{x_1}^{x_2} a_2\pab{x,y_\mathrm{top}}  \odif{x} - \int_{x_1}^{x_2} a_2\pab{x,y_\mathrm{btm}} \odif{x}
     & = \int_{\partial S_\mathrm{top}} a_2 n_2  \odif{l} + \int_{\partial S_\mathrm{btm}} a_2 n_2  \odif{l} \notag \\
     & = \oint_{\partial S} a_2 n_2 \odif{l}
  \end{align}
  $x$成分についても同様に$\displaystyle \int_{S} \pdv{a_1}{x} \odif{S} = \oint_{\partial S} a_1 n_1 \odif{l}$が示せるため，両者の和をとることで\eqref{eq:Gauss-Divergence-Theorem-2D}が得られる．
\end{proof}
...
\begin{FormulaBox}{発散の積の微分}{Identity-Div-Product}
  ...
  \end{equation}
  これは，1変数の積の微分公式 $\pab{uv}' = u'v + uv'$ の多次元版に相当する．
\end{FormulaBox}

\begin{proof}
  直交座標系における成分計算によって示す．$\nabla$演算子の$i$成分を$\partial_i = \pdv{}/{x_i}$，$\vect{A}$の第$i$成分を$A_i$と表記し，アインシュタインの縮約記法を用いる．
