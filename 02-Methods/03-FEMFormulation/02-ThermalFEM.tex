      ...
      \item[(c) Robin 境界条件]
            ...
            \begin{align}
              ...
              + c_\mathrm{w} \rho_\mathrm{w} \vect{j}_\mathrm{WL}\pab{\vect{x}, t} \cdot \vect{n}\, T\pab{\vect{x}, t} \notag   \\
               & \quad + c_\mathrm{v} \rho_\mathrm{w} \vect{j}_\mathrm{WV}\pab{\vect{x}, t} \cdot \vect{n}\, T\pab{\vect{x}, t}
...
% \subsubsection{完全連成解析における時間離散化と修正Picard反復法}
% \label{Sec:TimeDiscretization_Picard}

% 熱移動支配方程式 \eqref{Eq:ThermalGoverning_Final} に対して，BDFを用いた時間離散化を行う．
% 変数 $X$ の時刻 $t_{n+1}$ における時間微分項は，BDF係数 $\alpha_k$ を用いて以下のように近似される．
% \begin{equation}
%   \pdv{X}{t} \bigg|_{n+1} \approx \alpha_0 X_{n+1} + X_\mathrm{hist}, \quad X_\mathrm{hist} = \sum_{k=1}^{k} \alpha_k X_{n+1-k}
% \end{equation}
% ここで，$X_\mathrm{hist}$ は過去の時刻の既知量から定まる履歴項である．

% 本解析では，温度 $\vect{T}$ と間隙水圧 $\vect{P}_\mathrm{w}$ を同時に解く完全連成解析（Monolithic Approach）を採用し，非線形解法には修正Picard反復法を用いる．
% 第 $m$ 回目の反復における更新解 $\vect{T}^{(m+1)}, \vect{P}_\mathrm{w}^{(m+1)}$ を求めるための線形方程式系は，以下のブロック行列形式で記述される．

% \begin{equation}
%   \label{Eq:Monolithic-Picard-System}
%   \begin{bmatrix}
%     \mat{K}_\mathrm{TT}^{(m)} & \mat{K}_\mathrm{TH}^{(m)} \\
%     \mat{K}_\mathrm{HT}^{(m)} & \mat{K}_\mathrm{HH}^{(m)}
%   \end{bmatrix}
%   \begin{pmatrix}
%     \vect{T}^{(m+1)} \\
%     \vect{P}_\mathrm{w}^{(m+1)}
%   \end{pmatrix}
%   =
%   \begin{pmatrix}
%     \vect{F}_\mathrm{T, RHS}^{(m)} \\
%     \vect{F}_\mathrm{H, RHS}^{(m)}
%   \end{pmatrix}
% \end{equation}

% ここで，熱移動支配方程式に対応する上段のブロック（$\mat{K}_\mathrm{TT}, \mat{K}_\mathrm{TH}$）および右辺ベクトル $\vect{F}_\mathrm{T, RHS}$ は，前節で定義した一般化熱容量 $C_\mathrm{TT}, C_\mathrm{TH}$ を用いて以下のように構築される．

% \begin{enumerate}
%   \item \textbf{熱－熱ブロック $\mat{K}_\mathrm{TT}$（対角ブロック）} \\
%         温度変化に対する慣性項（見かけの熱容量）と熱伝導項から構成される．
%         \begin{align}
%           \mat{K}_\mathrm{TT}^{(m)}
%            & = \iiint_{V} \alpha_0 C_\mathrm{TT}^{(m)} \vect{\psi}_\mathrm{T} \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{V}
%           + \iiint_{V} \nabla\vect{\psi}_\mathrm{T}^\mathsf{T} \tensor{R}^{(m)} \nabla\vect{\psi}_\mathrm{T} \odif{V} \notag \\
...
%           + \iint_{\partial V_\mathrm{HR}} \alpha_\mathrm{HR}^{(m)} \vect{\psi}_\mathrm{T} \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{S}
%         \end{align}

%   \item \textbf{熱－水ブロック $\mat{K}_\mathrm{TH}$（非対角ブロック）} \\
%         水圧変化が熱収支に与える影響を表す連成項であり，左辺行列に組み込むことで強連成を実現する．
%         \begin{equation}
%           \mat{K}_\mathrm{TH}^{(m)} = \iiint_{V} \alpha_0 C_\mathrm{TH}^{(m)} \vect{\psi}_\mathrm{T} \vect{\psi}_\mathrm{P}^\mathsf{T} \odif{V}
%         \end{equation}

%   \item \textbf{熱等価外力ベクトル $\vect{F}_\mathrm{T, RHS}$} \\
%         境界条件からの既知の流入出，非線形性の強い移流項，および時間微分の履歴項から構成される．
%         \begin{align}
%           \vect{F}_\mathrm{T, RHS}^{(m)}
%            & = - \iint_{\partial V_\mathrm{HN}} q_\mathrm{N} \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{S}
%           - \iint_{\partial V_\mathrm{HC}} \gamma \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{S} \notag                                                                    \\
%            & \quad + \iint_{\partial V_\mathrm{HH}} h T_\mathrm{env} \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{S}
%           + \iint_{\partial V_\mathrm{HR}} \alpha_\mathrm{HR}^{(m)} T_\mathrm{r} \vect{\psi}_\mathrm{T}^\mathsf{T} \odif{S} \notag                                     \\
%            & \quad + \iiint_{V} \nabla\vect{\psi}_\mathrm{T}^\mathsf{T} \cdot \vect{j}_\mathrm{adv}^{(m)} \odif{V} \notag                                              \\
%            & \quad - \iiint_{V} \vect{\psi}_\mathrm{T}^\mathsf{T} \left( C_\mathrm{TT}^{(m)} T_\mathrm{hist} + C_\mathrm{TH}^{(m)} P_\mathrm{w, hist} \right) \odif{V}
%         \end{align}
%         ここで，$\vect{j}_\mathrm{adv}^{(m)}$ は前回の反復値で評価された，流体移動に伴う顕熱および潜熱フラックスの総和である．
%         \begin{equation}
%           \vect{j}_\mathrm{adv}^{(m)} = \left( c_\mathrm{w} \rho_\mathrm{w} \vect{j}_\mathrm{WL} T + c_\mathrm{v} \rho_\mathrm{w} \vect{j}_\mathrm{WV} T + \rho_\mathrm{w} L_\mathrm{v} \vect{j}_\mathrm{WV} \right)^{(m)}
%         \end{equation}
% \end{enumerate}