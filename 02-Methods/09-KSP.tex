...
\begin{algorithm}[H]
  \caption{Preconditioned Bi-CGSTAB method (Theoretical)}
  \label{PreBiCGSTAB}
  \begin{algorithmic}[1]
...
\begin{algorithm}[H]
  \caption{Implementation of preconditioned Bi-CGSTAB method}
  \label{Alg:IMP_PreBiCGSTAB}
  \begin{algorithmic}[1]
...
\begin{algorithm}[H]
  \caption{Preconditioned GMRES(m) method (Right Preconditioning)}
  \label{Alg:PreFullGMRES_Corrected}
...
\begin{algorithm}[H]
  \caption{Implementation of right-preconditioned GMRES(m) method (based on LIS)}
  \label{Alg:IMP_LIS_FGMRES}
  \setlength{\baselineskip}{17pt}
  \begin{algorithmic}[1]
    \State Set an initial value $\vect{x}_0$
    \State Compute $\vect{r} = \vect{b} - \mat{A} \vect{x}_0$
