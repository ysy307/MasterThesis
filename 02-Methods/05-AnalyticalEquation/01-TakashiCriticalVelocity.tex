...
凍土の成長に伴い，隣接する凍結管から広がる凍土同士の間隔が狭まると，上流側と下流側の水位差（Dam-up head）が増大する（\cref{Fig:freezing_model}a）．この水位差，凍結管の配置，および土壌の物性値の相互作用により，最終的に凍土壁が閉塞できるかどうかが決定される．
\begin{figure}[tbp]
  \centering
  \begin{tikzpicture}[
...
\end{figure}
ここで，地下水流れがある場合の凍土の凍結面の形状を円形とし，凍結管の間の地下水流れを一様な速度断面として考える．また，Dam-up headは凍土壁が完成した時の値を用い，それは凍土壁の前後で凍結管ピッチ$L$の距離だけ離れた二点$A$と$B$に生じると仮定する．
...
\begin{equation}
  ...
  \odif{\Pw}=
  \begin{cases}
    \displaystyle \dfrac{Q_\mathrm{w}}{K_\mathrm{s}} \dfrac{R\sin\theta}{L-2R\sin\theta}\odif{\theta} & -R < x < R                           \\[4mm]
...
\begin{align}
  P_\mathrm{A} - P_\mathrm{B}
   & = - \int_{x=-L/2}^{x=L/2}\odif{\Pw} \notag                                                                                                                                                                                                                                             \\
   & = - \int_{x=-L/2}^{x=-R}\odif{\Pw} - \int_{x=-R}^{x=R}\odif{\Pw} - \int_{x=R}^{x=L/2}\odif{\Pw} \notag                                                                                                                                                                                 \\
   & = - \int_{x=-L/2}^{x=-R}\odif{\Pw} - \int_{\theta=\pi}^{\theta=0} \odif{\Pw} - \int_{x=R}^{x=L/2}\odif{\Pw} \notag                                                                                                                                                                     \\
   & = \dfrac{Q_\mathrm{w}}{K_\mathrm{s}} \int_{x=-L/2}^{x=-R} \dfrac{1}{L}\odif{x} + \dfrac{Q_\mathrm{w}}{K_\mathrm{s}} \int_{\theta=0}^{\theta=\pi} \dfrac{R\sin\theta}{L-2R\sin\theta}\odif{\theta}  + \dfrac{Q_\mathrm{w}}{K_\mathrm{s}} \int_{x=R}^{x=L/2} \dfrac{1}{L}\odif{x} \notag \\
   & = \dfrac{Q_\mathrm{w}}{K_\mathrm{s}}\bab{\int_{\theta=0}^{\theta=\pi}\dfrac{R\sin\theta}{L-2R\sin\theta}\odif{\theta} +　\dfrac{2}{L} \int_{x=-L/2}^{x=R}\odif{x}}\label{Eq:Takashi_pressure_difference}
\end{align}
\eqref{Eq:Takashi_pressure_difference}の最初の積分部分は，ワイエルシュトラス置換を使えば
\begin{align*}
  \int_{\theta=0}^{\theta=\pi}\dfrac{R\sin\theta}{L-2R\sin\theta}\odif{\theta}
   & = \int_{\theta=0}^{\theta=\pi}\pab{- \dfrac{1}{2} + \dfrac{L/2}{L-2R\sin\theta}}\odif{\theta}                                   \\
   & = \bab{-\dfrac{1}{2}\theta}_{0}^{\pi} + \int_{t=0}^{t=\infty} \dfrac{L/2}{L-2R\pab{\dfrac{2t}{1+t^2}}} \dfrac{2}{1+t^2}\odif{t} \\
   & = -\dfrac{\pi}{2} + \int_{t=0}^{t=\infty} \dfrac{L}{L\pab{1+t^2}-4Rt} \odif{t}
\end{align*}
凍結が進行している場合，$L>2R$であるので，判別式$D=4R^2-L^2>0$となり，$b=2R/L$とおけば，$t$の広義積分は\footnote{$\displaystyle \int \dfrac{1}{x^2+a^2} \odif{x}=\dfrac{1}{a}\arctan\dfrac{x}{a}+C$を用いている．}
\begin{align*}
  \int_{t=0}^{t=\infty} \dfrac{L}{L\pab{1+t^2}-4Rt} \odif{t}
   & =\int_{t=0}^{t=\infty} \dfrac{L}{L t^2-4Rt+L} \odif{t}                                      \\
   & = \int_{t=0}^{t=\infty} \dfrac{1}{\pab{t-\dfrac{2R}{L}}^2+1-\pab{\dfrac{2R}{L}}^2} \odif{t} \\
   & = \int_{t=0}^{t=\infty} \dfrac{1}{\pab{t-b}^2+1-b^2} \odif{t}                               \\
   & = \dfrac{1}{\sqrt{1-b^2}} \bab{\arctan\dfrac{t-b}{\sqrt{1-b^2}}}_{0}^{\infty}               \\
   & = \dfrac{1}{\sqrt{1-b^2}} \pab{\dfrac{\pi}{2} + \arctan\dfrac{b}{\sqrt{1-b^2}}}             \\
\end{align*}
したがって，\eqref{Eq:Takashi_pressure_difference}は
\begin{align}
  P_\mathrm{A} - P_\mathrm{B} & =\dfrac{Q_\mathrm{w}}{K_\mathrm{s}}\bab{1-b+\dfrac{1}{\sqrt{1-b^2}}\pab{\dfrac{\pi}{2}+\arctan\dfrac{b}{\sqrt{1-b^2}}}-\dfrac{\pi}{2}} \notag \\
                              & = \dfrac{Q_\mathrm{w}}{K_\mathrm{s}}\mathcal{F}\pab{b}
\end{align}
ここで，関数$\mathcal{F}\pab{b}$は，
\begin{equation}
  \mathcal{F}\pab{b}=1-b+\dfrac{1}{\sqrt{1-b^2}}\pab{\dfrac{\pi}{2}+\arctan\dfrac{b}{\sqrt{1-b^2}}}-\dfrac{\pi}{2}
\end{equation}
と定義される．

次に凍結管との熱交換によって変化した地下水流の温度の計算を行う．\cref{Fig:freezing_model}bで示すように，上流部の水の温度を$T_\infty$，凍結管を流れた水の温度を$T_\mathrm{down}$とすれば，地下水流は凍結管の間を通過する際に$T_\infty$から$T_\mathrm{down}$に変化する．このとき，地下水流の熱量減少量$Q_\mathrm{wh}$は
\begin{equation}
  \label{Eq:Takashi_heat_flow}
  Q_\mathrm{wh}=Q_\mathrm{w}C_\mathrm{p}\pab{T_\infty-T_\mathrm{down}}=\dfrac{K_\mathrm{s}C_\mathrm{p}\pab{P_\mathrm{A}-P_\mathrm{B}}\pab{T_\infty-T_\mathrm{down}}}{\mathcal{F}\pab{b}}
\end{equation}
...
\begin{align}
  T_\mathrm{down} & =T_\infty-\dfrac{2\pi \lambda_\mathrm{f} \pab{T_\mathrm{f}-T_\mathrm{pipe}}}{K_\mathrm{s}C_\mathrm{p}\pab{P_\mathrm{A}-P_\mathrm{B}}}\dfrac{\mathcal{F}\pab{b}}{\log\dfrac{L}{2a}-\log\dfrac{L}{2R}}\notag \\
                  & =T_\infty-\dfrac{2\pi \lambda_\mathrm{f} \pab{T_\mathrm{f}-T_\mathrm{pipe}}}{K_\mathrm{s}C_\mathrm{p}\pab{P_\mathrm{A}-P_\mathrm{B}}}\mathcal{F}\pab{\dfrac{2a}{L}, b}
\end{align}
ここで，$\mathcal{F}\pab{2a/L,b}$は$2a/L<b<1$で唯一の最大値を持つ．そこで，$T_\mathrm{down}$が最大となる$b$を$b_\mathrm{crit}$とすれば
\begin{equation}
  T_\mathrm{down}^{\max}=T_\infty-\dfrac{2\pi \lambda_\mathrm{f} \pab{T_\mathrm{f}-T_\mathrm{pipe}}}{K_\mathrm{s}C_\mathrm{p}\pab{P_\mathrm{A}-P_\mathrm{B}}}\mathcal{F}\pab{\dfrac{2a}{L}, b_\mathrm{crit}}
\end{equation}
...
\begin{equation}
  ...
  \pab{P_\mathrm{A}-P_\mathrm{B}}_\mathrm{crit}=\dfrac{2\pi \lambda_\mathrm{f} \pab{T_\mathrm{f}-T_\mathrm{pipe}}}{K_\mathrm{s}C_\mathrm{p}}\mathcal{F}\pab{\dfrac{2a}{L}, b_\mathrm{crit}}
\end{equation}
本研究では特に平面凍結壁の場合を考えるので，その時の限界流速を$V_\mathrm{crit}$とすれば，
\begin{equation}
  \label{Eq:Takashi_critical_velocity_Linear}
  V_\mathrm{crit}=\dfrac{2\pi \lambda_\mathrm{f}}{l C_\mathrm{p}}\dfrac{\pab{T_\mathrm{f}-T_\mathrm{pipe}}}{\pab{T_\infty-T_\mathrm{f}}}\mathcal{F}\pab{\dfrac{2a}{L}, b_\mathrm{crit}}
\end{equation}
