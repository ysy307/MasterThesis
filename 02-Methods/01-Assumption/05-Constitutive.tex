...
\end{equation}
ここで，土粒子，間隙水，間隙氷，間隙空気，有機物の各相の体積熱容量 $C_\mathrm{s}$， $C_\mathrm{w}$， $C_\mathrm{ice}$， $C_\mathrm{v}$， $C_\mathrm{o}$ \unit{\joule.\meter^{-3}.\kelvin^{-1}} を，それぞれの比熱 (\eqref{Eq:Density_TP})と密度 (\eqref{Eq:Specific_Heat_TP})の積として次のように定義する．
\begin{subequations}
...
一方，不飽和土壌における熱伝導率は，非線形性が強く，様々な経験式が提案されている．本研究では，\textcite{Campbell-1985}が提案した式を凍土に拡張した\textcite{Hansson-2004}の式を採用する．
\begin{equation}
...
さらに，熱伝導率の異方性を考慮すれば，\eqref{Eq:Thermal_Conductivity_Saturated}または\eqref{Eq:Thermal_Conductivity_Unsaturated}を用いて見かけの熱伝導率テンソル$\lambda_{ij}$ \unit{\watt.\meter^{-1}.\kelvin^{-1}} は次式で与えられる．
\begin{equation}
...
\end{equation}
ここで，$\lambda_\mathrm{L}$，$\lambda_\mathrm{T}$ はそれぞれ縦方向（longitudinal）および横方向（transverse）の熱分散長 \unit{\meter} を表す．$\vect{q} = \pab{q_1, q_2, q_3}$は液状水フラックス\unit{\meter.\second^{-1}}，$\norm{\vect{q}}_2$ はダルシー流束密度の大きさ \unit{\meter.\second^{-1}}，$\delta_{ij}$ は Kroneckerのデルタ(\cref{Form:Kronecker-delta})である．記号 $\norm{\cdot}_2$ はユークリッドノルムを意味し，次元数を $dim$ とすると，
\begin{equation*}
  \norm{\vect{q}}_2 = \sqrt{\sum_{i=1}^{\mathrm{dim}} q_i^2}
\end{equation*}
...
ここで， 正規化飽和度 $\Sw$ \unit{-} は，体積含水率 $\Qw$，残留体積含水率 $\theta_\mathrm{r}$ \unit{\meter^{3}.\meter^{-3}}，飽和体積含水率 $\theta_\mathrm{s}$ \unit{\meter^{3}.\meter^{-3}} を用いて次式で定義される．
\begin{equation}
...
\begin{description}
  % [label=\arabic*：, leftmargin=*, labelwidth=1.5em, labelsep=0.5em]
  \item[Brooks-Corey (BC) モデル：] \mbox{} \\
        \textcite{Brooks-1964}によるWRFおよびHCFは，以下で与えられる．
        \begin{subequations}
          \begin{equation}
            \label{Eq:Brooks-Corey-WRF}
            \Sw =
            \begin{cases}
  ...
  \item[van Genuchten-Mualem (vG) モデル：] \mbox{} \\
        ...
        \begin{subequations}
          ...
          \end{equation}
          \begin{equation}
            \label{Eq:van-Genuchten-Mualem-HCF}
            k_\mathrm{r} = \Sw^{l}\bab{1-\pab{1-\Sw^{1/m}}^m}^2
          \end{equation}
        \end{subequations}
        \label{Eq:van-Genuchten-Mualem}
        ここで，$m = 1 - 1/n, \pab{n>1}$ の関係が用いられる．
  \item[Kosugi (KO) モデル：] \mbox{} \\
  ...
  \item[Durner モデル：] \mbox{} \\
        ここまでに示したモデルでは捉えきれない団粒構造などを表現するため，\textcite{Durner-1994}は複数のWRFを線形結合する手法を提案した．
        \begin{equation}
          \label{eq:Linear_Combination}
          \Sw = \sum_{i=1}^{k} w_i S_{\mathrm{w}_i}
        \end{equation}
        ここで，$S_{\mathrm{w}_i}$ は $i$番目のサブモデルの有効水分量である．
        ...
        \begin{subequations}
          ...
          \begin{equation}
            \label{Eq:Durner-HCF}
            \begin{split}
              k_\mathrm{r} = & \dfrac{\pab{w_1 S_{\mathrm{w}_1} + w_2 S_{\mathrm{w}_2}}^l}{(w_1 \alpha_1 + w_2 \alpha_2)^2}                                             \\
                             & \times \Bab{w_1 \alpha_1 \bab{1-\pab{1-S_{\mathrm{w}_1}^{1/m_1}}^{m_1}} + w_2 \alpha_2 \bab{1-\pab{1-S_{\mathrm{w}_2}^{1/m_2}}^{m_2}}}^2
            \end{split}
          \end{equation}
        \end{subequations}
        ここで $w_i$ は各サブモデルの重み係数$\pab{\sum w_i = 1}$であり，$\alpha_i$，$n_i$，$m_i$ はそれぞれ $i$番目のサブモデルの空気侵入圧，間隙径分布指数，および $m_i = 1 - 1/n_i$ の関係である．
  \item[Dual-vG-CH モデル：] \mbox{} \\
        \textcite{Seki-2022}は，Durnerモデルのパラメータを削減するため，各vGサブモデルで共通のパラメータ $\alpha$ を用いるDual-vG-CHモデルを提案した．
        \begin{subequations}
          \begin{equation}
            \label{Eq:Dual-vG-CH-WRF}
            \Sw = w\pab{1+\abs{\alpha h}^{n_1}}^{-m_1} + \pab{1-w}\pab{1+\abs{\alpha h}^{n_2}}^{-m_2}
          \end{equation}
          \begin{equation}
            \label{Eq:Dual-vG-CH-HCF}
            \begin{split}
              k_\mathrm{r} = & \dfrac{\pab{w S_{\mathrm{w}_1} + \pab{1-w} S_{\mathrm{w}_2}}^l}{\pab{w \alpha + \pab{1-w} \alpha}^2}                                     \\
                             & \times \Bab{w \alpha \bab{1-\pab{1-S_{\mathrm{w}_1}^{1/m_1}}^{m_1}} + \pab{1-w} \alpha \bab{1-\pab{1-S_{\mathrm{w}_2}^{1/m_2}}^{m_2}}}^2
            \end{split}
          \end{equation}
        \end{subequations}
\end{description}

また，液状水が凍結する際に，氷が形成されることにより，液状水が移動可能な間隙体積が減少する．この現象を考慮するため，間隙氷量 $\Qice$ を用いて，透水係数は次式で修正される\parencite{Taylor-1978}．
\begin{equation}
  \label{Eq:Hydraulic_Conductivity_Ice}
  K_\mathrm{flh} = K_{\mathrm{s}} 10^{-\Omega \Qice}
\end{equation}
ここで，$K_\mathrm{flh}$は凍結水含有状態における透水係数 \unit{\meter.\second^{-1}}，$\Omega$ は経験的な抵抗定数 \unit{-}であり，本研究では $\Omega = 10$ を採用した．
