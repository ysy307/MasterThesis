...
\begin{equation}
  ...
  h = - \frac{2 \sigma_\mathrm{surf} \cos\alpha_\mathrm{c}}{\rho_\mathrm{w} g r_\mathrm{cap}}
\end{equation}
ここで，$\sigma_\mathrm{surf}$ は土壌間隙水の表面張力 \unit{\newton.\meter^{-1}}，$\alpha_\mathrm{c}$ は接触角 \unit{\radian}，$r_\mathrm{cap}$ は毛管半径 \unit{\meter} である．
$\sigma_\mathrm{surf}$は温度の関数として次式で表される．
\begin{equation}
  \label{Eq:SurfaceTension}
  \sigma_\mathrm{surf}\pab{T\,} = 75.6 - 0.1425 T - 2.38 \times 10^{-4} T^2
\end{equation}
マトリックポテンシャル $h$ の温度依存性を調べるため，$\sigma_\mathrm{surf}$と$\rho_\mathrm{w}$が温度に依存するとし，\eqref{Eq:MatricPotential}を$T$ で偏微分すると，次式が得られる．
\begin{align}
  \label{Eq:MatricPotentialGradient}
  \pdv{h}{T} & = - \frac{2 \cos\alpha_\mathrm{c}}{g r_\mathrm{cap}} \frac{\displaystyle \rho_\mathrm{w}\pdv{\sigma_\mathrm{surf}}{T} - \sigma_\mathrm{surf}\pdv{\rho_\mathrm{w}}{T}}{\rho_\mathrm{w}^2} \notag \\
             & = \frac{\rho_\mathrm{w} h}{\sigma_\mathrm{surf}} \frac{\displaystyle \rho_\mathrm{w}\pdv{\sigma_\mathrm{surf}}{T} - \sigma_\mathrm{surf}\pdv{\rho_\mathrm{w}}{T}}{\rho_\mathrm{w}^2} \notag     \\
             & = h \pab{\frac{1}{\sigma_\mathrm{surf}}\pdv{\sigma_\mathrm{surf}}{T} - \frac{1}{\rho_\mathrm{w}}\pdv{\rho_\mathrm{w}}{T}} \notag                                                                \\
             & \approx h \frac{1}{\sigma_\mathrm{surf}}\pdv{\sigma_\mathrm{surf}}{T}
\end{align}
ここで，簡略化するために，水の密度 $\rho_\mathrm{w}$ の温度依存性を無視した．
よって $\vect{j}_\mathrm{WL}$は，温度勾配 $\nabla T$ とマトリックポテンシャル勾配 $\nabla h$ の関数として次式で表される．
\begin{equation}
  \label{Eq:LiquidFlux_final2}
  \vect{j}_\mathrm{WL} = - K_\mathrm{w} \pab{\frac{h}{\sigma_\mathrm{surf}}\pdv{\sigma_\mathrm{surf}}{T} \nabla T + \nabla h + \nabla z}
\end{equation}
...
\begin{align}
  ...
  \label{Eq:KwT_def}
  K_\mathrm{wT} & = K_\mathrm{w} h G_\mathrm{wT} \frac{1}{\sigma_\mathrm{surf}}\pdv{\sigma_\mathrm{surf}}{T}
\end{align}
ここで，$G_\mathrm{wT}$は液相水の温度勾配に起因するフラックスの増進係数 \unit{-} であり，砂質土では通常$7$とする．
...
ここで，飽和水蒸気密度$\rho_\mathrm{v}^\mathrm{sat}$ \unit{\kilogram.\meter^{-3}} は温度$T$の関数であると近似でき，次のように表される．
\begin{equation}
