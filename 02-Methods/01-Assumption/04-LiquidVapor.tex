...
\end{figure}
また，水蒸気密度$\rho_\mathrm{v}$は，飽和水蒸気密度$\rho_\mathrm{v}^\mathrm{sat}$ \unit{\kilogram.\meter^{-3}} と相対湿度$H_\mathrm{r}$の積で表される．
\begin{align}
...
\end{align}
さらに，水蒸気を液水換算したときの体積水蒸気量 $\Qv$ \unit{-} は，水蒸気密度 $\rho_\mathrm{v}$ と気相率 $\Qa$ を用いて次のように定義される．
\begin{equation}
...
ここで例えば温度を$\qty{298.15}{\kelvin}=\qty{25.0}{\degreeCelsius}$と固定すれば，\eqref{Eq:RelativeHumidityFinal}の指数部の$h$以外の部分$\pab{M_\mathrm{w} g / \pab{R T^\ast}}$を定数として計算でき，その定数部はおよそ $\qty{7.13e-5}{\meter^{-1}}$ となる．
ここで，$h>\qty{-1.0e3}{\meter}$であれば，指数部の絶対値が比較的小さいため，$H_\mathrm{r}$は$1$に非常に近い値をとる．
しかし，$h$ が $\qty{-1.0e3}{\meter}$ より小さくなる（負の方向に大きくなる）と，指数部の$h$の影響が大きくなり，$H_\mathrm{r}$は減少し始める．$h<-\qty{1.0e6}{\meter}$ になると$h$が支配的になり，$H_\mathrm{r}$は$0$に漸近する．この傾向は \cref{Fig:Hr_soil} と一致する．

一方，絶対温度 $T^\ast$は\eqref{Eq:RelativeHumidityFinal}の指数の分母にある．$h$ は負であるため，$T^\ast$ が上昇すると$H_\mathrm{r}$は大きくなる．
...
土壌が極めて乾燥している状態（例えば $h < \qty{-2.0e3}{\meter}$）や塩分濃度が高い状態を除けば，$H_\mathrm{r} \approx 1$ の近似を用いることができる．
...
このとき，基準マトリックポテンシャルを $\psi_\mathrm{m,ref} = \qty{-2.0e3}{\meter}$ のように適切に設定すれば，$h > \psi_\mathrm{m,ref}$ の範囲において $H_\mathrm{r} \approx 1$ と近似できることが確認される．
