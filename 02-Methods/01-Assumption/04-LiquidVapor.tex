...
土壌表面などある基準水面を考えれば，その上部に存在する水は毛管力によって引き上げられ，基準水面よりも高い位置にある水は負圧状態になる．
ここで，水のポテンシャルを基準水面からの水柱の高さで表したものがマトリックポテンシャル $h$ \unit{[\meter]} である．
このとき，マトリックポテンシャル $h$ の場所にある液相の水の比自由エネルギー $\Delta f_\mathrm{w}$ \unit{[\joule.\kilogram^{-1}]} は，次のようになる．
\begin{equation}
  \label{Eq:LiquidFreeEnergy}
  \Delta f_\mathrm{w} = h g
\end{equation}
ここで，$g$は重力加速度 \unit{[\meter.\second^{-2}]} である．
一方，同じ高さにおける水蒸気の比自由エネルギー $\Delta f_\mathrm{v}$ \unit{[\joule.\kilogram^{-1}]} は，その場所の蒸気圧$P_\mathrm{v}$と基準水面$h=0$での飽和蒸気圧$P_\mathrm{v}^\mathrm{sat}$との比で決定される．
\begin{equation}
...
考えている土壌の系が熱力学的平衡状態にあるとき，液相と気相の単位質量あたりのHelmholtz自由エネルギーは等しい．すなわち，$\Delta f_\mathrm{w} = \Delta f_\mathrm{v}$が成り立つ．これを用いれば，\eqref{Eq:LiquidFreeEnergy}と\eqref{Eq:VaporFreeEnergy}より次式（Kelvin方程式）が得られる．
\begin{equation}
  \label{Eq:LiquidVaporEquilibrium}
  h g =\frac{R T^\ast}{M_\mathrm{w}} \ln{\frac{P_\mathrm{v}}{P_\mathrm{v}^\mathrm{sat}}}
\end{equation}
ここで，相対湿度$H_\mathrm{r}$は，空気中の水蒸気分圧と飽和水蒸気圧の比で表されるので，
\begin{equation}
...
\end{figure}
ここで，水蒸気密度$\rho_\mathrm{v}$は，飽和水蒸気密度$\rho_\mathrm{v}^\mathrm{sat}$と相対湿度$H_\mathrm{r}$の積で表される．
\begin{align}
...
しかし，$h$ が $\qty{-1e3}{\meter}$ より小さくなる（負の方向に大きくなる）と，指数部の$h$の影響が大きくなり，$H_\mathrm{r}$は減少し始める．$h<-\qty{e6}{\meter}$ になると$h$が支配的になり，$H_\mathrm{r}$は$0$に漸近する．この傾向は Fig.~\ref{fig:Hr_soil} と一致する．
一方，絶対温度 $T^\ast$は\eqref{Eq:RelativeHumidityFinal}の指数の分母にある．$h$ は負であるため，温度 $T^\ast$ が上昇すると，$H_\mathrm{r}$は大きくなる．
しかし，$T^\ast$の変動幅が $h$ の変動幅に比べて小さいため，Fig.~\ref{fig:Hr_soil_contour}に示す通り，$H_\mathrm{r}$の$T^\ast$に対する感度は $h$ に対する感度と比べて小さい．
...
前者の条件下では温度依存性を無視しうるため，Fig.~\ref{fig:Hr_soil_validation} に示すように，温度 $\qty{15}{\degreeCelsius}$ における $H_\mathrm{r}$ と $h$ の関係を代表例として用いることができる．
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth,pagebox=cropbox,clip]{2-1/Hr_contour_plot.pdf}
  \caption{温度およびマトリックポテンシャルに対する相対湿度の応答曲面}\label{fig:Hr_soil_contour}
\end{figure}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth,pagebox=cropbox,clip]{2-1/Hr_soil_validation.pdf}
