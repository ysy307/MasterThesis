...
本研究で対象とする凍土は，熱・水分移動を同時に取り扱う必要があるため，多相混合体として定式化する．ここでは，土粒子（solid），液相水（liquid），氷（ice），および気相（air）の四相からなる系を考える\cref{Fig:4Phase_Soil}．
\begin{figure}[!b]
  \centering
  ...
  \end{tikzpicture}
  \caption{凍土中における四相地盤構造}
  \label{Fig:4Phase_Soil}
\end{figure}
このとき間隙比$e$ \unit{-} および間隙率$\phiv$ \unit{-} は次式で定義される．
\begin{subequations}
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
間隙体積に対する各相の体積の比率である飽和度 $S_\alpha$ \unit{-} を次式で定義する．ただし，$\alpha = \mathrm{n}\!:\text{soil},\ \mathrm{w}\!:\text{water},\ \mathrm{i}\!:\text{ice},\ \mathrm{a}\!:\text{air},\ \mathrm{v}\!:\text{vapor}$である．
\begin{subequations}
...
\begin{equation}
  ...
  \Sw + \Sice + \Sa = 1
\end{equation}
また，全体体積に対する体積分率$\theta_\alpha$ \unit{\meter^3.\meter^{-3}} は次式で定義される．
\begin{subequations}
  \label{Eq:Theta_Def}
  \begin{align}
...
各相（固相，液相，気相）における熱力学状態は，温度$T$，圧力$p$，および比体積$\nu$を基本変数として定義する．本節の表記は\textcite{Tasaki-Thermodynamics}に従う．
...
\begin{equation}
  ...
  F = U - TS
\end{equation}
ただし，Helmholtzの自由エネルギー$F$が$T$について微分可能とする．その全微分は，\eqref{Eq:dU}および\eqref{Eq:Helmholtz}を用いて次式で導かれる．
\begin{align}
  \odif{F}
   & = \odif{U} - \odif{(TS)} \notag                                                       \\
...
\begin{equation}
  ...
  H = U + pV
\end{equation}
その全微分は\eqref{Eq:dU}より，次式で与えられる．
\begin{align}
  \odif{H}
   & = \odif{U} + \odif{\pab{pV}} \notag                                                   \\
...
\begin{equation}
  ...
  G = U - TS + pV
\end{equation}
$G$ は $F$ （\eqref{Eq:Helmholtz}） または $H$（\eqref{Eq:Enthalpy_def}）を用いて次のようにも表される．
\begin{equation}
  \label{Eq:ThermoRelation}
  G = F + pV = H - TS
\end{equation}
$G$ の全微分は\eqref{Eq:dF}より次式で与えられる．
\begin{align}
  \odif{G}
   & = \odif{F} + \odif{(pV)} \notag                                                        \\
...
\end{align}
物質量 $N$ が一定（$\odif{N}=0$）の場合，\eqref{Eq:dG}は次式のように書ける．
\begin{equation}
  \label{Eq:dG_constN}
  \eval{\odif{G}}{{N=\text{const}}} = -S \odif{T} + V \odif{p}
\end{equation}
また，各相の物性は温度と圧力の関数として次式で表される．
\begin{subequations}
  \begin{align}
    \label{Eq:Density_TP}
  ...
  \end{align}
\end{subequations}
特に後章で導入する体積熱容量$C$や熱伝導率$\lambda$の導出において，これらの関数形を用いる．
